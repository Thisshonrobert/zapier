...
- **AI triage boundary**: Phases 4A–4C provide the implemented read-only, owner-scoped evidence path, and Phase 5 adds six simulated runbooks with bounded deterministic retrieval. Use [architecture.md](./architecture.md) for the authenticated service boundary, [current-state.md](./current-state.md) for evidence/redaction/retrieval and verification details, and [ai-dlq-master-plan.md](./ai-dlq-master-plan.md) for phase scope. Phase 4 contracts remain preserved; Phase 6 integrated diagnosis, graph integration, approval, replay, and frontend work remain deferred.
